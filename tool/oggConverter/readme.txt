■使い方
aoTuV(http://www.geocities.jp/aoyoume/aotuv/)を利用してwavファイルをoggに変換します。
動作には上記ウェブページで配布しているvenc.exeが必要です。

ogg変換.batと同じフォルダにvenc.exeとkrkr.exeを置いてください。
それからogg変換.batを起動すると変換ウィンドウが表示されます。
左の欄で変換後の音質を選択し、wavファイルをウィンドウにドロップすると変換が開始されます。
変換後のファイルは変換前のファイルと同じ場所に作成されます。



■音質設定について
qの後の数字が大きいほど音質がよくなります。
mp3のビットレートとおおよその対応は以下のようになります。
...
■ライセンス
startup.tjs, icon_16.ico, icon_48.icoはパブリックドメインです。自由に使ってください。特に条件はありません。
...
■利用しているライブラリ等
・kwidgets(https://sv.kikyou.info/svn/kirikiri2/trunk/kwidgets)
・scriptsEx(https://sv.kikyou.info/svn/kirikiri2/trunk/kirikiri2/src/plugins/win32/scriptsEx)
・shellExecute(https://sv.kikyou.info/svn/kirikiri2/trunk/kirikiri2/src/plugins/win32/shellExecute)
