...
何もないウィンドウが出てくるので、KAGEXで書かれたksファイルをドロップしてください。
ページごとにラベルが挿入されたファイルが作成されます。

ksファイルはラインモードが"vn", "tex", "free"のいずれかの構文で書かれている必要があります。



■ライセンス
startup.tjs, はパブリックドメインです。自由に使ってください。特に条件はありません。
...
■利用しているライブラリ等
・saveStruct(https://sv.kikyou.info/svn/kirikiri2/trunk/kirikiri2/src/plugins/win32/saveStruct)
